import React from "react";  // Every React component or page needs to import React
import { nanoid } from "nanoid";    // It will help us to generate the shorter URL, the last poertion and it also makes sure that it is URL safe in terms special characters or spaces
import { getDatabase, child, ref, set, get } from "firebase/database"    // It is required, since we are going to use the Firebase database
import { isWebUri } from "valid-url";   // To validate the Url
...
class Form extends React.Component {
    ...
    // Handle the input, when user clicks "Submit" button
    handleSubmit = async (event) => {
        ...
        var generatedURL = "shortURL.com/" + generatedKey

        if (this.state.preferedAlias !== '') {
            generatedKey = this.state.preferedAlias
            generatedURL = "shortURL.com/" + this.state.preferedAlias
        }
    ...
    // Checks if any specific field has an error
    checkErr = (event) => {
        return this.state.errors.indexOf(event) !== -1;
    }


    // On changes, this function updates the content of the form
    handleChange = (event) => {
    ...
    validateInput = async () => {
        ...
        if (this.state.longURL.length === 0) {
            errors.push("longURL");
            errorMessages['longURL'] = "The URL is not Valid, too less characters. Please check and re-enter the URL!";
        } else if (!isWebUri(this.state.longURL)) {
            errors.push("longURL");
            errorMessages['longURL'] = "Is not a valid URL, please enter an URL in the form of https://www.";
        } else if (this.state.longURL.length <= 6) {
            errors.push("longURL");
            errorMessages['longURL'] = "The URL is not Valid, it is shorter than the Short URL. Please check and re-enter the URL!";
        }

        //Prefered Alias
        if (this.state.preferedAlias !== '') {
            if (this.state.preferedAlias.length > 8) {
        ...
        return (
            <div className="container">
                <form autoComplete="off">
                    ...
                    <div
                        className={
                            this.checkErr("longURL") ? "text-danger" : "visually-hidden"
                        }
                    >
                    ...
                    <div className="form-group">
                        <label htmlFor="basic-url">Enter Your Prefered URL</label>
                        <div className="input-group mb-3">
                            ...
                            <input
                                ...
                                className={
                                    this.checkErr("preferedAlias")
                                        ? "form-control is-invalid"
                                        : "form-control"
                                }
                                type="text" placeholder="eg. e8ni1 [Optional]"
                            />
                        </div>
                        <div
                            className={
                                this.checkErr("suggestedAlias") ? "text-danger" : "visually-hidden"
                            }
                        >
